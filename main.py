...
TEXTS = {
    'uk': {
        ...
        'roles': "🎭 Ролі", 'anketa': "📝 Подати анкету", 'rules_btn': "📜 Правила", 'news_btn': "🏎 Трансляції та новини",
        ...
        'step1': "Крок 1: Введіть повне ім'я вашого персонажа:", 'step2': "Крок 2: Введіть ваш вік:",
        'success': "✨ Вашу анкету успішно надіслано!", 'admin_text': "НОВА АНКЕТА НА ВСТУП",
        'char': "Персонаж", 'user': "Користувач", 'age': "Вік", 'accept': "✅ Схвалити", 'decline': "❌ Відхилити", 'review': "⏳ На розгляді"
    },
    'ru': {
        ...
        'roles': "🎭 Роли", 'anketa': "📝 Подать анкету", 'rules_btn': "📜 Правила", 'news_btn': "🏎 Трансляции и новости",
        'news_msg': "🏁 Свежие новости и бесплатные трансляции Формулы 1 доступны по ссылке:\n👉 @f1_racing_translation",
        'info': "F1 Flood\n--------------------------\nШАБЛОН АНКЕТЫ:\n\n1. Полное имя желаемого персонажа.\n2. Ваш возраст.\n--------------------------",
        'step1': "Шаг 1: Введите полное имя вашего персонажа:", 'step2': "Шаг 2: Введите ваш возраст:",
        'success': "✨ Ваша анкета успешно отправлена!", 'admin_text': "НОВАЯ АНКЕТА НА ВСТУПЛЕНИЕ",
        'char': "Персонаж", 'user': "Пользователь", 'age': "Возраст", 'accept': "✅ Одобрить", 'decline': "❌ Отклонить", 'review': "⏳ На рассмотрении"
    }
}
...
def set_language(call):
    ...
    buttons = [types.KeyboardButton(TEXTS[lang]['roles']), types.KeyboardButton(TEXTS[lang]['anketa']), 
               types.KeyboardButton(TEXTS[lang]['rules_btn']), types.KeyboardButton(TEXTS[lang]['news_btn'])]
    if call.message.chat.id == MY_ID: buttons.append(types.KeyboardButton("📢 Розсилка"))
    markup.add(*buttons)
    bot.send_message(call.message.chat.id, TEXTS[lang]['welcome'], reply_markup=markup)

@bot.message_handler(func=lambda message: message.text == "📢 Розсилка" and message.from_user.id == MY_ID)
...
def send_roles(message):
    try: bot.copy_message(message.chat.id, CHANNEL_ROLES_ID, 4)
    except: bot.send_message(message.chat.id, "❌ Помилка: Не вдалося отримати ролі.")
...
def handle_admin_buttons(call):
    ...
    if action == "accept":
        ...
        markup.add(types.InlineKeyboardButton("❌ Не підходить вік", callback_data=f"reason_age_{target_user_id}_{call.message.message_id}"),
